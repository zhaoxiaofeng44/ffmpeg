...
LWFNode::~LWFNode()
{
	_destructed = true;
	if (lwf) {
		shared_ptr<NS_LWF::Data> data = lwf->data;
		lwf->Destroy();
...
bool LWFNode::initWithLWFFile(const string &path, void *l)
{
	...
	lwf->FitForWidth(data->header.width,data->header.height);

	Size tempSize = Size(data->header.width,data->header.height);
	
	setContentSize(tempSize);
	setAnchorPoint(Point::ZERO);
	setVertexRect(Rect(0.0f,0.0f,tempSize.width,tempSize.height));
